...
export default function Booking() {
    ...
    const handleBookingConfirm = async (e) => {
        ...
        const datePlusHour = () => {
            ...
            const datePart = selectedDate.toLocaleString("default");

            const dateOnly = datePart.split(' ')[0];

            const convertedDate = dateOnly.split('/');

            const concatenatedDates = `${convertedDate[2]}-${convertedDate[1]}-${convertedDate[0]}`;

            const datePlusHour = `${concatenatedDates}T${selectedHour}:00.000Z`;
    ...
    const formatedDate = `${jours[selectedDate.getDay() - 1]
